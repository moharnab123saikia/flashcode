...
class FlashcardProvider extends ChangeNotifier {
  ...
  Future<void> resetProgress() async {
    ...
    try {
      // Reset all flashcards to original state (no reviews, default difficulty)
      final resetCards = _flashcards.map((card) {
        return card.copyWith(
          personalDifficulty: 2, // Reset to default
          reviewCount: 0,
          easeFactor: 2.5,
          interval: 1,
          lastReviewedAt: null,
      ...
      // Save the reset cards
      await SyncService.instance.saveFlashcards(resetCards);
      
      // Update local state
      _flashcards = resetCards;
      _applyFilters();
      notifyListeners();
  ...
  // Clear all data and reload fresh dataset
